...
gwst promotes working directories into explicit **workspaces (task-scoped working directories)** and enables Git worktrees to be operated in a **standardized, safer, and listable** way.  
gwst focuses on **creating and managing work environments**; downstream development workflows (run, test, PR, etc.) are intentionally left to the user’s existing practices.

## 2. Who this tool is for (Target users)

gwst is primarily for developers and teams who work in the terminal and build productivity by composing existing tools (git/gh/tmux/make/just/direnv, etc.).
...
gwst provides a minimal set of management capabilities: **create, extend, list, and safely clean up** workspaces.  
