...
## Intent
Create the minimum directory and file layout under the resolved GWST root so other commands can operate safely.
...
- Creates `gwst.yaml` if it does not exist, seeding it with:
...
- `gwst.yaml` exists and is a file (either newly created or previously present).

## Failure Modes
- Root directory not provided or cannot be accessed.
- Filesystem errors while creating directories or writing `gwst.yaml`.
