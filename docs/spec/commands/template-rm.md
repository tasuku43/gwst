---
title: "gwst template rm"
status: implemented
---

## Synopsis
`gwst template rm [<name> ...]`
...
- Requires `templates.yaml` to exist (`gwst init` completed). Missing file => error.
- With names provided:
...
- UI behavior mirrors `gwst template add` selection: the highlighted item is added on `<Enter>` and removed from the candidate list; a minimum of 1 selection is required.
