---
title: "gwst template add"
status: implemented
---

## Synopsis
`gwst template add <name> [--repo <repo> ...]`
...
- Validation:
  - `templates.yaml` must exist (i.e., `gwst init` already run); otherwise error.
...
- After selection completes, the command writes `templates.yaml` and renders the standard section blocks consistent with other gwst commands (no header line; section “Result” lists the template name and repos).
