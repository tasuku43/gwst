...
## Synopsis
`gwiac manifest validate [--root <path>] [--no-prompt]`

## Intent
Validate `gwiac.yaml` (the desired-state inventory) for schema correctness and core invariants, producing actionable errors suitable for humans and CI.

This is a read-only command. It does not scan the filesystem and does not run `gwiac apply`.
...
- Validates each repo entry in `workspaces.<id>.repos[]`:
  ...
  - `alias` must be unique within the workspace and must not be `.gwiac`.
...
- Presets:
  - Preset entries are validated by `gwiac manifest preset validate`.
...
## Related behavior
- `gwiac plan` must fail (non-zero exit) when `gwiac.yaml` is invalid; it must not print a plan in that case.

## Success Criteria
- Returns success when `gwiac.yaml` is valid.
