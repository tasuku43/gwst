...
## Synopsis
`gwiac manifest rm [<WORKSPACE_ID> ...] [--no-apply] [--no-prompt]`

## Intent
Remove workspace entries from the inventory (`gwiac.yaml`) using an interactive UX, then reconcile the filesystem via `gwiac apply` by default.
...
- Updates `<root>/gwiac.yaml` by removing the selected workspace entries.
- By default, runs `gwiac apply` to reconcile the filesystem with the updated manifest.
  - Destructive behavior is enforced by `gwiac apply` (and `--no-prompt` must error if removals exist).
- With `--no-apply`, stops after rewriting `gwiac.yaml` and prints a suggestion to run `gwiac apply` next.
...
2. Validate targets:
   - Each selected ID must exist in `gwiac.yaml` (inventory-driven removal).
   - If any selected ID is missing from `gwiac.yaml`, the command errors and makes no changes.
3. Rewrite `gwiac.yaml` (full-file rewrite) to remove the selected workspace entries.
4. If `--no-apply` is set: stop after manifest rewrite.
5. Otherwise run `gwiac apply` for the entire root:
   - This may include unrelated drift in the same root.
   - Destructive confirmation rules are handled by `gwiac apply`.
6. If apply is cancelled at the confirmation step (`n`/No or `Ctrl-C`), restore the previous `gwiac.yaml` from a backup/snapshot.
...
When apply runs, `gwiac manifest rm` should emit an `Info` section after `Inputs` to make the two-phase behavior explicit:
- `manifest: updated gwiac.yaml (removed N workspace(s))`
...
- Detailed risk output should primarily come from the `gwiac apply` plan output (same format as `gwiac plan`), so users can review before confirming destructive removals.
...
### Output: `--no-apply`
When `--no-apply` is set, `gwiac manifest rm` does not run apply and prints a short summary.
...
Result
  • updated gwiac.yaml (removed 1 workspace)
...
When apply runs, `gwiac manifest rm` prints `Inputs` first, then streams `gwiac apply` output (`Info`/`Plan`/`Apply`/`Result`).
`gwiac manifest rm` itself does not attempt to summarize the plan beyond what `gwiac apply` prints.

## Success Criteria
- Selected workspace entries are removed from `gwiac.yaml`.
- When apply is run and confirmed, filesystem no longer contains removed workspaces.

## Error messages (guidance)
`gwiac manifest rm` should keep errors actionable and include the next command when possible.

Common cases:
- `--no-prompt` with no `<WORKSPACE_ID>` args: error and suggest providing args or removing `--no-prompt`.
- Any selected workspace ID is missing from `gwiac.yaml`: error and include the missing ids (no changes are made).
...
- Any selected workspace ID is missing from `gwiac.yaml` (no changes are made).
