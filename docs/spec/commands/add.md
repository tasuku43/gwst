...
## Behavior
- Accepts optional `WORKSPACE_ID` and `repo`; if either is missing, interactively prompts the user to choose a workspace and a repo store discovered via `gws repo ls`. Fails if no candidates exist.
...
- Opens the bare store without fetching (expects `gws repo get` to have been run already).
...
- Repo store missing (user must run `gws repo get` first).
