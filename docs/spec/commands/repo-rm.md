...
## Intent
Remove one or more bare repo stores under the gwst root, only if unused by any workspace.

## Behavior
- Accepts zero or more repo specs (SSH/HTTPS), same format as `gwst repo get`.
...
- With no specs and prompts allowed:
  - Loads existing stores via `gwst repo ls` and opens a filterable multi-select list.
  - Selection UX matches `gwst template rm`: `<Enter>` selects, removes from candidates; finish with `<Ctrl+D>` or `done`; minimum 1 selection required.
