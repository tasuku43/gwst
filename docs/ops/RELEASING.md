---
title: "Releasing gion"
status: implemented
---

# Releasing gion (v0.1.0)

This document describes how we ship gion as a “touchable OSS” with a reliable install path.
...
- `gion version` (and `gion --version`) shows `vX.Y.Z` in the GitHub Releases binaries.

## Versioning and build metadata

`gion version` output is populated via `-ldflags` at build time:
